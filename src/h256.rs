...
impl H256 {
    ...
    pub fn copy_bits(&self, start: u8) -> Self {
        ...
        let end_byte = (ARRAY_SIZE - start / BYTE_SIZE) as usize;
        // copy bytes
        target.0[..end_byte].copy_from_slice(&self.0[..end_byte]);

        // reset remain bytes
        let remain = start % BYTE_SIZE;
        if remain > 0 {
            target.0[end_byte - 1] &= 0b11111111 << remain
        }
